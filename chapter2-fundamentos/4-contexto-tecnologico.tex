...
con el framework \textit{Expo} React Native, un backend entermente gestionado por \textit{Supabase}, con PostgreSQL como motor de bases de datos, 
...
\subsection{Capa del Cliente Móvil}
...
\subsection{Capa de Datos y Persistencia Local}
...
mientras que Expo ofrece \acrshort{api}s para el almacenamiento persistente en el dispositivo mediante \textit{AsyncStorage} o sistemas 
...
\subsection{Capa de Servicios Externos}
...
distribuidas globalmente en edge nodes cercanos a los usuarios. La arquitectura de gateway implementada por \textit{Supabase} sigue el 
siguiente flujo de procesamiento:

[INCLUIR IMAGEN DE LA ARQUITECTURA DE EDGE FUNCTIONS]

\begin{enumerate}
    \item \textbf{Entrada de solicitud en el edge gateway}: El \gls{api-gateway} actúa como relay que enruta el tráfico, gestiona los 
    headers de autenticación, valida los JSON Web Tokens (\acrshort{jwt}) y aplica reglas de enrutamiento y control de tráfico.
    
    \item \textbf{Aplicación de autenticación y políticas}: El \gls{api-gateway} valida los \acrshort{jwt} de \textit{Supabase}, aplica 
    ...
    \item \textbf{Ejecución en el edge runtime}: La función se ejecuta en un nodo de \textit{Edge Runtime} distribuido regionalmente 
    más cercano al usuario, minimizando la latencia de procesamiento.
    
    \item \textbf{Integraciones y acceso a datos}: Las funciones comúnmente invocan \acrshort{api}s de \textit{Supabase} (Auth, Postgres, Storage) 
    o \acrshort{api}s de terceros, utilizando estrategias de conexión optimizadas para entornos edge/serverless.
    
    \item \textbf{Observabilidad y logs}: Las invocaciones emiten logs y métricas que pueden explorarse en el dashboard de 
    \textit{Supabase} o integrarse con sistemas de monitorización downstream como \textit{Sentry}.
...
Esta arquitectura de \gls{api-gateway} [COMO SE PUEDE VER EN LA FIG.N] proporciona múltiples beneficios que justifican su adopción:
...
\subsection{Capa de Sistema de Pagos}
...
de pasarela de pagos. \textit{Stripe} es una plataforma líder que proporciona \acrshort{api}s robustas y seguras para procesar pagos, 
...
como \gls{pci-dss} (Payment Card Industry Data Security Standard).
...
con Kit de Desarrollo de Software (\acrshort{sdk}s) específicos para iOS y Android. \textit{Polar} no cuenta con suficiente adopción en el ecosistema de aplicaciones móviles, 
...
\subsection{Capa de Observabilidad}
...
abierto que proporciona lwa capacidad de hacer un seguimiento de eventos una vez la aplicación está en manos del usuario.
...
usuarios con un \acrshort{sdk} robusto para React Native, y \textit{Amplitude}, una plataforma líder en análisis de productos que proporciona 
capacidades avanzadas de \gls{analisis-cohorte} y \gls{embudos}. Sin embargo, ambas alternativas presentan modelos de precios más restrictivos 
