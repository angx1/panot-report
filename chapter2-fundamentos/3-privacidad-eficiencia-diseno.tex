...
En este contexto, la privacidad no requiere ocultar información al usuario propietario y governador del dato, sino protegerlos 
...
A nivel de infraestructura, se ha utilizado \textit{Supabase} como plataforma de backend como ya se explicará en puntos posteriores. 
\textit{Supabase} tiene la particularidad de autogestionar estas medidas de seguridad a través de dos mecanismos: 
Por un lado, todos los datos se encriptan en reposo mediante \gls{aes-256} y en tránsito mediante el protocolo \gls{transport-layer-security}, 
garantizando la protección de la información en todas las etapas del ciclo de vida. Y por otro lado, \textit{Supabase} proporciona 
\gls{row-level-security} y \gls{column-level-security}, permitiendo un control granular del acceso a los datos a nivel de 
fila y columna, lo que asegura que cada usuario solo pueda acceder a sus propios datos y a nada más. \textit{Supabase} además cumple 
...
manteniendo la máxima calidad en las funcionalidades principales -aunque esta filosofía, que guiará todo el desarrollo, 
se explicará más en detalle en los capítulos siguientes-.
...
esencial del sistema —la gestión inteligente de relaciones interpersonales mediante modelos de lenguaje— mientras se 
optimiza el consumo de recursos computacionales y económicos asociados con el alojamiento de la infraestructura y 
