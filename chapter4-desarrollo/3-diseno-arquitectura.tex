...
\begin{itemize}
    ...
    PostgreSQL con \acrshort{rls}, y sincronización en tiempo real, 
...
{\small
\begin{itemize}
    \item \textbf{Autenticación}: Email con \acrfull{otp} y Apple Sign-In, con gestión 
          automática de tokens \acrshort{jwt}.
    \item \textbf{Base de datos}: Persistencia de datos en PostgreSQL con políticas \acrshort{rls} para aislamiento 
...
\begin{figure}[h]
    ...
    \caption{Diagrama de la arquitectura del sistema multi-agente de PANOT. En naranja se representa el esquema de la petición enviada al sistema, en morado el agente orquestador, en rosa los agentes especializados y en azul las tablas abstraídas de la base de datos.}
    \label{fig:diagrama-multi-agente}
\end{figure}

\newpage

El sistema Multi-Agente de PANOT sigue una arquitectura vertical \cite{mohamed-fadi-2025-agentic-ai}, donde un agente orquestador actúa como coordinador central, delegando 
...
nuevo con una arista {\footnotesize \texttt{RELACIONADO\_CON}} hacia $n^*$, preservando la conexión semántica. Y en el 
...
o cuando sus nodos están enlazados mediante una arista {\footnotesize \texttt{RELACIONADO\_CON}} 
...
\begin{figure}[ht!]
    ...
    En el diagrama del schema \textit{<<auth>>} solo se ha incluido la tabla \textit{auth.users} ya que es la única tabla de el 
...
El modelo de datos de PANOT se organiza en dos schemas: \textit{<<auth>>} (autenticación y gestión de usuarios gestionado por Supabase) y \textit{<<public>>} (lógica de negocio). 
