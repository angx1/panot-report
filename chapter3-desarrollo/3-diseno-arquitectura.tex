...
\begin{figure}[ht!]
    ...
    \caption{Diagrama del modelo de datos de PANOT del schema de Supabase \textit{<<auth>>}. Solo se incluye la tabla \textit{auth.users} ya 
    que es la unica tabla de el schema \textit{<<auth>>} que se ha modificado.}
...
\begin{figure}[ht!]
    ...
    \caption{Diagrama del modelo de datos de PANOT del schema de Supabase \textit{<<public>>}. \textbf{Nota:} Se ha obviado del diagrama 
    que las claves foráneas \texttt{user\_id} o \texttt{owner\_id} de las tablas provienen del \texttt{id} de la tabla \texttt{users} del schema \textit{<<auth>>}. A modo de ejemplo, 
    solo se ha añadido en la tabla \texttt{profiles} para mantener el diagrama más legible.}
