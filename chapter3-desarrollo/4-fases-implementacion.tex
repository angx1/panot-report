...
El primer mes de desarrollo se centró en establecer la identidad del usuario y la capacidad de gestión de contactos e interacciones local. El objetivo 
...
el trabajo en las operaciones clave del \acrshort{crud} de contactos que quedaban pendientes. Se implementó 
...
y palabras clave (Issue \#11), y la eliminación permanente de contactos (Issue \#9). Esto completó la experiencia básica de gestión de contactos.
...
dictado en lenguaje natural. Se desarrollaron mejoras en la visualización de audio y se implementó el flujo completo de creación de 
contactos "hablando sobre ellos" (Issue \#37, PRs \#38, \#40).

\paragraph{Configuraciones de la Aplicación (\hyperref[req:FR-16]{FR-16}, \hyperref[req:FR-17]{FR-17})} Se añadió la pantalla de 
ajustes de la aplicación con configuraciones como el idioma de voz y otras preferencias del usuario (Issue \#42, PRs \#44, \#45). 
Esta funcionalidad estableció la base para futuras capacidades de personalización y el apartado de feedback.
...
mecanismo de autenticación robusto y funcional.
...
constituyó el primer paso hacia el sistema de inteligencia relacional completo.

{\footnotesize
\textit{Nota}: todavía no se contaba con el sistema multi-agente ni la clase de grafo semántico, es decir, todavía no existía un sistema de procesamiento relacional 
...
el sistema multi-agente, el sistema de cola de procesamiento, la instrumentación de observabilidad y los mecanismos de soporte. El objetivo fue alcanzar 
...
\paragraph{Gestión de Pagos y Suscripciones} Se completó la lógica de \textit{paywall} e integración con \textit{Stripe} para suscripciones mensuales 
...
\begin{enumerate}
    ...
    \item Integración de \textit{LangSmith} como capa de observabilidad del sistema agéntico, habilitando el seguimiento y depuración de 
    las trazas de ejecución de los agentes, permitiendo monitorizar el flujo de procesamiento con información como latencia, tokens utilizados y coste de la ejecución.
\end{enumerate}
...
\paragraph{Capa de Observabilidad (\hyperref[req:RNF-MANT-008]{RNF-MANT-008})} Se instrumentó la aplicación para análisis y telemetría 
utilizando herramientas como \textit{PostHog} (Epic \#55). Esta implementación permitió monitorizar el comportamiento de los usuarios y 
las funcionalidades del sistema, cumpliendo con el requisito de captura de métricas para validar las hipótesis de valor definidas en el proyecto.
...
de errores, sugerencias y solicitudes desde la aplicación (Issue \#43). Esta funcionalidad estableció el canal de comunicación entre 
...
Habiendo finalizado el desarrollo completo del \acrshort{mvp}, principios de enero (del 6 al 8) de 2026 se dedicaron únicamente al desarrollo de la 
página web landing de PANOT tanto en inglés como en español. En la web se han incluido las siguientes secciones:

{\small
\begin{itemize}
    \item \textbf{Inicio}: Pagina de inicio de la web donde se expone la hipótesis de valor y el link de descarga de la aplicación.
    \item \textbf{Changelog}: Línea de tiempo de cambios de PANOT.
    \item \textbf{Política de Privacidad}: Política de privacidad de PANOT. Necesario para la publicación de la aplicación en los canales de distribución.
...
\begin{figure}[ht!]
    \centering
    \includegraphics[width=1\textwidth]{figures/landing-home-1.png}
    \caption{Página de inicio de la landing de PANOT (parte 1)}
    \label{fig:landing-home-1}
\end{figure}

\vspace{.5cm}
\begin{figure}[ht!]
    \centering
    \includegraphics[width=1\textwidth]{figures/landing-changelog.png}
    \caption{Página de changelog de la landing de PANOT}
    \label{fig:landing-changelog}
\end{figure}

\vspace{.5cm}
\begin{figure}[ht!]
    \centering
    \includegraphics[width=1\textwidth]{figures/landing-privacy.png}
    \caption{Página de política de privacidad de la landing de PANOT}
    \label{fig:landing-privacy}
\end{figure}

\vspace{.5cm}
\begin{figure}[ht!]
    \centering
    \includegraphics[width=1\textwidth]{figures/landing-terms.png}
    \caption{Página de términos y condiciones de la landing de PANOT}
    \label{fig:landing-terms}
\end{figure}
