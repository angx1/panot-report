...
\begin{enumerate}
    ...
    \setlength{\topsep}{0pt}
    \setlength{\partopsep}{0pt}
    \item \textbf{Valor de la Captura sin Fricción:} Se asume que los usuarios solo registrarán información de sus contactos si el esfuerzo requerido es 
    cercano a cero. La captura mediante una interfaz \textit{voice-first} proporciona este valor al permitir registrar matices humanos al instante y en 
    movimiento, eliminando la barrera del teclado y permitiendo que el usuario mantenga su atención en la interacción social.
    
    \item \textbf{Valor de la Continuidad Relacional:} Se asume que el usuario obtiene una utilidad crítica al disponer de una cronología estructurada 
    (\textit{timeline}) de sus encuentros. Este registro histórico permite retomar conversaciones de forma natural, eliminando la 
    incertidumbre del \textit{dónde lo dejamos} y transformando una lista estática de nombres en una historia relacional viva.
    
    \item \textbf{Valor del Entendimiento Contextual:} Se asume que el valor real de un contacto no reside en sus datos demográficos, sino en su contexto único y su 
    relación con el usuario. Es por eso que un contacto pasa de ser una entidad tabular a ser una persona con un contexto específico que va evolucionando a lo largo del tiempo.
\end{enumerate}
\label{sec:valor-hipotesis}
}


\subsection{Historias de Usuario}

Para traducir estas hipótesis en funcionalidades concretas y mantener un enfoque centrado en el usuario, se han definido una serie de Historias de 
Usuario (\acrshort{hu}). Estas historias actúan como unidades de trabajo que describen el \textit{quién}, el \textit{qué} y el \textit{para qué} de cada capacidad del sistema. \\
...
\textit{ID} & \textit{Historia de Usuario} \\ \hline
    \textbf{HU-01} \label{req:HU-01} & \textit{Como} usuario, \textit{quiero} registrarme y acceder de forma segura \textit{para} mantener mis relaciones personales organizadas y accesibles de manera segura y privada. \\ \hline
    \textbf{HU-02} \label{req:HU-02} & \textit{Como} usuario, \textit{quiero} importar contactos de mi agenda nativa \textit{para} poder usar mis contactos ya existentes. \\ \hline
    \textbf{HU-03} \label{req:HU-03} & \textit{Como} usuario, \textit{quiero} crear un contacto dictando su descripción o escribiéndolo manualmente \textit{para} permitir que ellos mismos lo hagan o hacerlo con el mínimo esfuerzo posible. \\ \hline
    \textbf{HU-04} \label{req:HU-04} & \textit{Como} usuario, \textit{quiero} grabar una interacción por voz \textit{para} poder reducir el esfuerzo de registrarla y para hacerlo a mayor velocidad que escribiendo. \\ \hline
    \textbf{HU-05} \label{req:HU-05} & \textit{Como} usuario, \textit{quiero} validar o corregir la información procesada por el sistema \textit{para} asegurar que el registro de mis relaciones sea siempre acorde a la realidad. \\ \hline
    \textbf{HU-06} \label{req:HU-06} & \textit{Como} usuario, \textit{quiero} visualizar un historial cronológico por cada contacto \textit{para} recordar instantáneamente el contexto de nuestras últimas conversaciones. \\ \hline
    \textbf{HU-07} \label{req:HU-07} & \textit{Como} usuario, \textit{quiero} buscar contactos por palabras clave del resumen \textit{para} localizarlos por su contexto y no solo por su nombre. \\ \hline
    \textbf{HU-08} \label{req:HU-08} & \textit{Como} usuario, \textit{quiero} que el sistema extraiga datos de interés de las interacciones que registre y actualice los perfiles de mis contacts automáticamente \textit{para} reducir el esfuerzo manual de edición de fichas. \\ \hline
    \textbf{HU-09} \label{req:HU-09} & \textit{Como} usuario, \textit{quiero} enviar reportes de error o sugerencias \textit{para} poder contribuir a la mejora del producto. \\ \hline
\end{tabularx}
\caption{Definición de las Historias de Usuario para el \gls{mvp} de PANOT}
...
con las capacidades para validar las premisas e historias de usuario planteadas. Estos requisitos de funcionalidad 
...
\rowcolor[HTML]{EFEFEF} 
\textit{ID}                            & \textit{Descripción} \\ \hline
\textbf{FR-16} \label{req:FR-16}       & El sistema debe disponer de una interfaz accesible desde el menú de configuración que permita al usuario enviar un reporte de error o sugerencia, incluyendo la descripción del problema. \\ \hline
...
Para garantizar la integridad del sistema y asegurar que cada desarrollo técnico responde directamente a una necesidad del usuario, se ha elaborado 
la siguiente matriz de trazabilidad. Esta herramienta vincula los \textit{Requisitos Funcionales} con las \textit{Historias de Usuario}. 
Esta trazabilidad permite verificar la cobertura total del \acrshort{mvp} y asegura que el diseño del sistema 
multi-agente y la arquitectura \textit{local-first} están plenamente justificados por el valor que aportan al usuario final.\\
...
\hline
\rowcolor[HTML]{EFEFEF} 
\textit{ID Requisito Funcional} & \textit{ID Historia de Usuario} \\ \hline
\hyperref[req:FR-01]{FR-01} & \hyperref[req:HU-01]{HU-01} \\ \hline
\hyperref[req:FR-02]{FR-02} & \hyperref[req:HU-01]{HU-01} \\ \hline
\hyperref[req:FR-03]{FR-03} & --- (necesidad técnica) \\ \hline
\hyperref[req:FR-04]{FR-04} & \hyperref[req:HU-03]{HU-03} \\ \hline
\hyperref[req:FR-05]{FR-05} & \hyperref[req:HU-02]{HU-02} \\ \hline
\hyperref[req:FR-06]{FR-06} & \hyperref[req:HU-03]{HU-03} \\ \hline
\hyperref[req:FR-07]{FR-07} & \hyperref[req:HU-07]{HU-07} \\ \hline
\hyperref[req:FR-08]{FR-08} & \hyperref[req:HU-03]{HU-03}, \hyperref[req:HU-05]{HU-05}, \hyperref[req:HU-08]{HU-08} \\ \hline
\hyperref[req:FR-09]{FR-09} & --- (necesidad técnica) \\ \hline
\hyperref[req:FR-10]{FR-10} & \hyperref[req:HU-04]{HU-04} \\ \hline
\hyperref[req:FR-11]{FR-11} & \hyperref[req:HU-05]{HU-05} \\ \hline
\hyperref[req:FR-12]{FR-12} & \hyperref[req:HU-04]{HU-04} \\ \hline
\hyperref[req:FR-13]{FR-13} & --- (necesidad técnica) \\ \hline
\hyperref[req:FR-14]{FR-14} & \hyperref[req:HU-08]{HU-08} \\ \hline
\hyperref[req:FR-15]{FR-15} & \hyperref[req:HU-08]{HU-08} \\ \hline
\hyperref[req:FR-16]{FR-16} & \hyperref[req:HU-09]{HU-09} \\ \hline
\hyperref[req:FR-17]{FR-17} & \hyperref[req:HU-09]{HU-09} \\ \hline
\end{tabular}
\caption{Matriz de Trazabilidad de Requisitos e Historias de Usuario}
\label{tab:matriz-trazabilidad-unica}
\end{table}
...
\end{tabularx}
\caption{Requisitos No Funcionales del sistema: ID, Tipo, Descripción y Criterio de Aceptación}
