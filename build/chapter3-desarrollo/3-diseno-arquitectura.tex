...
\begin{figure}[ht!]
    \centering
    \includegraphics[width=1\textwidth]{figures/db-modeled-auth.png}
    \caption{Diagrama del modelo de datos de PANOT del schema de Supabase \textit{<<auth>>}. Solo se incluye la tabla \textit{auth.users} ya que es la unica tabla de el schema \textit{<<auth>>} que se ha modificado.}
    \label{fig:diagrama-modelado-datos-auth}
\end{figure}


\begin{figure}[ht!]
    \centering
    \includegraphics[width=1\textwidth]{figures/db-modeled-public.png}
    \caption{Diagrama del modelo de datos de PANOT del schema de Supabase \textit{<<public>>}.}
    \label{fig:diagrama-modelado-datos-public}
\end{figure}

\newpage

El modelo de datos de PANOT se organiza en dos schemas: \texttt{<<auth>>} (gestionado por Supabase) y \texttt{<<public>>} (lógica de negocio). 
...
\begin{itemize}
    ...
    \texttt{<<auth>>} mediante \texttt{user\_id}. Contiene el estado del 
...
entidades relacionadas. Cabe añadir que este mecanismo ha sido incorporado de forma anticipada para futuras iteraciones del sistema.
...
A continuación, se presenta el diagrama de actividad de este sistea de procesamienton por cola. Para mantener la simplicidad se han obviado
datos como actualización de tiempos (\texttt{last\_activity\_at}, \texttt{processed\_at}, \texttt{created\_at}, \texttt{updated\_at}) y el tipo 
